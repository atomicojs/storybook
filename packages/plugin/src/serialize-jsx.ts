...
export function serializeJsx(vnode: any, deep = 0) {
    const children = [vnode].flat(100).map((vnode) => {
        ...
        if (localName) {
            ...
            return `${space}<${localName}${attrs ? ` ${attrs}` : ""}${
                children
                    ? `>${children}${
                          isBr ? "\n" + "    ".repeat(deep) : ""
                      }</${localName}>`
            ...
            if (getType(vnode) === "array") return serializeJsx(vnode);
        } else {
            return vnode;
        }
    });

    return children.filter((value: any) => value !== false).join("");
}
