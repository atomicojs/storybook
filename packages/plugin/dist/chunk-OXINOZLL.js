  ...
  if (child instanceof Element) {
    ...
    const attrs = [...props].filter(([prop]) => !ignore.includes(prop)).reduce((attrs2, [prop, value]) => {
      if (value != null) {
        ...
        const wrapper = type === "object" || type === "array" ? jsx ? ["{", "", "}"] : ["{", "", "}"] : ["", "", ""];
        let attr = type === "boolean" ? value ? prop : "" : type === "function" ? `${prop}=${type}` : `${prop}=${type === "object" || type === "array" ? `'${JSON.stringify(value)}'` : JSON.stringify(value)}`;
